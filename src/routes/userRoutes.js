...
function getFriendshipState(currentUserId, targetUserId) {
...
// POST /api/user/:id/friend-request
router.post('/:id/friend-request', requireAuth, async (req, res) => {
  try {
    ...
    const existingRelation = await getFriendshipState(currentUserId, targetUserId)
    ...
    const friendRequest =
      sameDirectionRequest?.status === 'DECLINED'
...
// GET /api/user/friend-requests/incoming
...
// GET /api/user/friend-requests/outgoing
...
// PATCH /api/user/friend-requests/:id/accept
router.patch('/friend-requests/:id/accept', requireAuth, async (req, res) => {
  try {
...
// PATCH /api/user/friend-requests/:id/decline
router.patch('/friend-requests/:id/decline', requireAuth, async (req, res) => {
  try {
    const requestId = parseUserId(req.params.id)
    if (!requestId) {
      return res.status(400).json({ message: 'Invalid friend request id' })
...
router.get('/friends', requireAuth, async (req, res) => {
  try {
    ...
    const friends = rows.map((row) => ({
      ...
      user: row.senderId === req.userId ? row.receiver : row.sender,
    }))
...
router.get('/:id/friend-status', requireAuth, async (req, res) => {
  try {
    ...
    const relation = await getFriendshipState(currentUserId, targetUserId)
